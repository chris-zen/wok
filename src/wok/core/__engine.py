...
from wok.core.flow import Flow
from wok.core.module import Module
from wok.core.task import Task
from wok.core.port import Port
...
class WokEngine(object):
	...
	def __init__(self, conf, flow_def = None):
		self.conf = conf
...
#		task["__doc_path"] = path
	...
	def _run(self):
		...
		self._log.info("Scheduling flow '%s' with %i modules ..." % (self._flow_def.name, len(self._mod_map)))
		...
		try:
			...
			while len(batch_modules) > 0:
				...
				for mnode in batch_modules:
					...
					mtasks = self._schedule_tasks(self._flow_def, mnode)
			...
			if len(failed_tasks) > 0:
				...
				self._log.error("Flow '%s' failed:\n%s" % (self._flow_def.name, msg))
				self._state = WokEngine.S_FAILED
			else:
				self._log.info("Flow '%s' finished successfully" % self._flow_def.name)
	...
	def start(self, async = True):
		...
		if self._state in [WokEngine.S_FINISHED, WokEngine.S_FAILED, WokEngine.S_EXCEPTION]:
			self._initialize(self._flow_def);
